...
5. Run `~/.shared/scripts/init-feature-folder.sh {name}` — creates folder structure with interview.yml.
...
Read ALL files from `.opencode/skills/project-knowledge/references/`. If directory missing or empty — warn user, suggest running project-planning skill (or `/init-project-knowledge` command).
...
1. Copy template to working file:
   - Copy `~/.shared/work-templates/user-spec.md.template` → `work/{feature}/user-spec.md`
